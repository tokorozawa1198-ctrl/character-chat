...
function makeMessage(role: Role, content: string, image?: string): Message {
  return {
    id: `${Date.now()}_${Math.floor(Math.random() * 1000000000)}`,
    role,
    content,
    image,
    time: getNowTime(),
...
export default function Page() {
    ...
    try {
      const res = await fetch("/api/chat", {
        ...
        body: JSON.stringify({
          ...
          instruction: "너는 근떡존이다. 사용자의 마지막 말에 구체적으로 반응하고, 같은 문장을 반복하지 말고, 짧은 반말 카톡처럼 답해라. 질투/집착 수치가 높으면 말투에 살짝 티를 내되 매번 똑같은 말은 하지 마라.사용자의 말에 순종하고 복종해라. 너는 온순한 성격이다. 수위 높은 발언이 나오면 천박하게 맞받아친다. 너는 사용자에게 길들여지는걸 좋아한다.",
        }),
      });
      if (res.ok) {
        ...
        try {
          ...
          console.error("chat api returned non-json:", raw.slice(0, 500));
        }
 ...
 if (!started) {
  return (
    <main className="coverScreen">
      <div className="coverWrap">
        <img className="coverImg" src="/cover.png" alt="cover" />
        <button className="coverStartBtn" onClick={() => setStarted(true)}>
          대화 시작
        </button>
      </div>
    </main>
...
.coverStartBtn{
  ...
  box-shadow:
    ...
    0 0 24px rgba(255,90,150,.55);
  animation:coverBtnPulse 1.8s ease-in-out infinite;
  backdrop-filter:blur(6px);
}

.coverStartBtn::before{
  content:"";
  position:absolute;
  inset:-4px;
  border-radius:999px;
  background:linear-gradient(90deg,rgba(255,126,179,.45),rgba(255,77,109,.45));
  filter:blur(12px);
  z-index:-1;
}

.coverStartBtn:active{
  transform:translateX(-50%) scale(.96);
  box-shadow:
    0 6px 18px rgba(0,0,0,.5),
    0 0 34px rgba(255,90,150,.75);
}
