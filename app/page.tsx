  ...
  async function sendMessage(forced?: string) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (candidate && !tooSimilar(candidate, last)) {
  reply = candidate;
}
    } catch {}
if (!reply) {
  ...
  function loadSlot(slot: number) { const raw = localStorage.getItem(SLOT_KEY(slot)); if (!raw) { alert(`${slot}번 슬롯 비어있음`); return; } const saved = JSON.parse(raw) as SaveData; setStats(saved.stats ?? initialStats); setMessages(saved.messages?.length ? saved.messages : [makeMessage("assistant", "불러왔어효. 다시 이어서 하자능.")]); setView(saved.view ?? "chat"); setCurrentScenarioId(saved.currentScenarioId ?? null); setSeenTriggers(saved.seenTriggers ?? {}); setCurrentPortrait(saved.currentPortrait ?? "/oppa1.png"); setGalleryTab(saved.galleryTab ?? "all"); }
